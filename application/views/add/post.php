...
$(document).ready(function() {
        ...
        $('.file_input').filer({
            ...
            templates: {
                ...
                item: '<li class="jFiler-item ui-state-default a">\
                ...
                itemAppend: '<li class="jFiler-item ui-state-default b">\
                            <div class="jFiler-item-container">\
                                <div class="jFiler-item-inner">\
                                    <div class="jFiler-item-thumb">\
                                        <div class="jFiler-item-status"></div>\
...
reader.onloadend = function(e) {
    var number = 1 + Math.floor(Math.random() * 6977);
    // make an image and append it to the div
    var single_img = '<div class="thumbnails" id="'+number+'"><img src="'+e.target.result+'" /></div>';
...
next: function(index) {
	if(index == 2 || index == 3 || index == 4){
        ...
        var acity = $("input[name=city]").val();
		add_title = add_title.charAt(0).toUpperCase() + add_title.substr(1);
		acity = acity.charAt(0).toUpperCase() + acity.substr(1);
		$('#product_title_and_price').html(add_title+" - "+acity+" $"+add_price);
		...
		$('#phone_number_here').html("(+1) "+phone);
		...
		*/
		...
		if(errors[1] == 1 && errors[2] == 1 && errors[3] == 1 && errors[4] == 1 && errors[5] == 1 && errors[6] == 1  &&  errors[9] == 1){
		return true;
		} else {
		return false;
		}
...
<div class="post-1 space">    
	...
	<form method="post" action="<?php echo base_url();?>add/post" enctype="multipart/form-data" id="custom" class="p2">
	...
	<legend>1. Details</legend>
			   ...
			   <div class="form1">
                ...
                <input type="hidden" name="dealer_type" value="<?php if(!empty($user[0]['user_type'])){echo ucfirst($user[0]['user_type']);}else {echo "Private";}?>"/>
                  ...
                  <p class="fifty">
                      ...
                      <span><label>Phone Number :</label><input type="number" placeholder=""  max="9999999999" min="1000000000"  name="phone_no" style="width:77% !important;" value="<?php echo $user[0]['telephone'];?>"/><span id="error_phone" class="error"></span></span>
				...
				<div class="form1">
				<p class="cat cat1 cat2"> <label>Posting Title :</label> <input type="text" class="category detail" name="add_title"/><span id="error_title" class="error"></span></p>
                                <p class="cat cat1"> <label>City </label><input type="text" class="category detail" name="city" value="<?php echo ucfirst($user[0]['city']);?>"/><span id="error_city" class="error"></span></p>
                  <p class="cat cat1"> <label>Price($) : </label><input type="number" min="1" class="category detail" name="price"/><span id="error_price" class="error"></span></p>
                  
                  <p class="cat cat1" style="margin-right: 0px !important;"> <label>Postal Code :</label> <input type="text" readonly="readonly" class="category detail" name="postal_code" value="<?php echo $user[0]['postal_code'];?>"/><span id="error_postal" class="error"></span></p>
                    ...
                    <p> <label>Posting Body :</label><span style="text-align:right">Please enter phone numbers as contact info above, not in posting body below.</span>
                     <textarea name="description" id="text_description"></textarea><span id="error_description" class="error"></span></p>
                     </div>
                     <div class="form1">
		...
		<span id="error_category" class="error"></span>
                ...
                Make / Manufacturer
                  ...
                  <input type="text" placeholder="Enter Year" name="year_make"/><span id="error_year_make" class="error"></span>
                </p-->
                 <p class="cat cat1">
                  <label>
                 Model Number
                  ...
                  <input type="text" placeholder="i.e 2010" name="model"/>
                </p>
                 <p class="cat cat1">
                  <label>
                 Size / Dimensions
                  ...
                  <input type="text" placeholder="i.e 100*100" name="size"/>
                </p>
                 <p class="cat cat1">
                  <label>
                 Condition
		...
		<p class="cat cat1">
                  <input type="checkbox" checked="checked" class="c2" name="show_on_maps" value="1"/> Show on maps
                </p>
		</div>
            </div>
			</div>
	...
	<legend>2. Upload Images</legend>
		...
		<div class="col-md-12 postad-photo-section">
			...
			<p>Drag and drop your photos to change the order. The photo in the first position will be your main photo.</p>
		</div>
        ...
        <fieldset class="tab_content" id="tab28" title="3. Publish">
	...
	<?php $user = $this->db->get_where('users',array('id'=>$_SESSION['user_id']))->result_array();?>
    	...
    	<div class="col-md-8 col-sm-8 col-xs-12">
        	...
        	<div class="product-detail">            	
            	<div class="main-iamge" id="add_main_image">
                	<img src="<?php echo str_replace('/index.php','',base_url());?>assets/images/side-bg.jpg" alt="product-Image" />
                </div>
            ...
            <div class="related-actions">
            	<div class="action-buttons">
                	<button class="btn btn-default btn-1" onclick="return false;" >Save</button>
                    <button class="btn btn-default" onclick="return false;">Compare</button>
                </div>
        ...
        <div class="col-md-4 col-md-4 col-sm-4 col-xs-12 eq">
				<div class="product-enquiry">
                   
            	<h3>Inquire About this Product</h3>
                <div class="call-wrap">
                <h1 id="phone_number_h1"><i class="fa fa-phone"></i> 
                    <span id="phone_number_here">999-999-9999</span>
                </h1>
              </div>
			...
			<?php } ?>
                ...
                <p class="seller"><br><span id="advertisement_address" style="border:none;"></span></p>
				<p class="view-web"><a target="_blank" href="<?php echo $user[0]['website_url'];?>">Visit our Website</a></p>
			</div>
       ...
       </div>   <input type="submit" class="finish" name="save_add" value="Publish" />  
       </div>
